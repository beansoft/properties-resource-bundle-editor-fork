...
open class MergingUpdateQueueFork @JvmOverloads constructor(
  ...
  private val restartSignal: MutableSharedFlow<Long>?
  private var collectJob: Job?
  private val legacyAlarm: Alarm?
  @Volatile
  ...
  init {
    ...
    if (coroutineScope != null) {
      restartSignal = MutableSharedFlow(extraBufferCapacity = 1, onBufferOverflow = BufferOverflow.DROP_OLDEST)
      legacyAlarm = null
      collectJob = coroutineScope.launch {
        @OptIn(FlowPreview::class)
        restartSignal.debounce { it }.collect {
          performCoroutineFlush()
        }
      }
  ...
  private fun restart(mergingTimeSpan: Int) {
    ...
    val signal = restartSignal
    if (signal != null) {
      hasPendingFlush = true
      signal.tryEmit(mergingTimeSpan.toLong())
    }
    else {
      legacyAlarm?.let { alarm ->
        hasPendingFlush = true
  ...
  private suspend fun performCoroutineFlush() {
    hasPendingFlush = false
...
//      if (update.executeInWriteAction) {
  ...
  override fun dispose() {
    try {
      ...
      finishActivity()
      collectJob?.cancel()
      legacyAlarm?.let { Disposer.dispose(it) }
      cancelAllUpdates()
    }
    finally {
      queues?.remove(this)
  ...
  fun waitForAllExecuted(timeout: Long, unit: TimeUnit) {
    ...
    while (hasPendingFlush || !scheduledUpdates.isEmpty) {
      ...
      Thread.sleep(minOf(10, maxOf(1, TimeUnit.NANOSECONDS.toMillis(toWait))))
    }
  }
}